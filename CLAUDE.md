...
- **Profile** (`src/features/profile/store.ts`) — name/department/mainProject in a Zustand `persist` store (localStorage). `ReportCreateView` pre-fills new reports from it (gated behind a mount check via `useSyncExternalStore` to avoid hydration mismatch on the filled inputs); both create and edit views call `setProfile(input.meta)` on save so it stays current. Saisir une fois, réutilisé chaque semaine.
...
- **Deploy note**: not serverless-compatible (needs LibreOffice + a persistent disk). Ships as a Docker image (`Dockerfile`, Next `output: "standalone"`) for Fly.io/VPS. `next.config.ts` sets `output: "standalone"`, `serverExternalPackages: ["better-sqlite3"]` (native module), and `outputFileTracingIncludes` for `templates/**`. Fly.io: `fly.toml` builds from the Dockerfile, mounts a volume at `/app/data`, single machine (SQLite single-writer), ≥1GB RAM for LibreOffice. The standalone bundle was verified booting outside Docker (`node .next/standalone/server.js`) with full CRUD + PPTX + PDF.
